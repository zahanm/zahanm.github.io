...
var fs, url, path, express, server, port;
...
server.get('/news/malkani/:year', function(req, res) {
  ...
  } else if (req.params.year === '2013' && req.query.auth === 'kj32n8i') {
    filename = 'static/news/malkani2013.html';
  } else {
    res.writeHead(404);
...
server.get('/sf', function(req, res) {
  var page = fs.createReadStream('static/sf-recommendations.html');
  res.writeHead(200, { 'Content-Type': 'text/html' });
  page.pipe(res);
});
